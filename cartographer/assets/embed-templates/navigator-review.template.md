...
Source: cartographer/commands/navigator/review.md
...
Arguments: `/spec-review <SPEC_FILE> [OPTIONS]`
...
Consider running `/spec-build {spec_file}` again after fixes.
```
...
2. Run `/spec-build {spec_file}` to re-implement
3. Run `/spec-review {spec_file}` to re-review
...
# Run /capture --from-staging to process
  ...
  - type: anti_pattern
    ...
    evidence:
      - file: src/controllers/UserController.ts
        line: 45
    suggested_rule: "Don't query database directly - use provider layer"
```
...
Options:
```
- "Capture all now" → Run `/capture --from-staging`
...
**To capture:** `/capture --from-staging`
```
