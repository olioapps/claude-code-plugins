...
Arguments: `/atlas-review [OPTIONS]`
...
|---------|----------------|
| `/validate` | Structure/format (links valid, required fields present) |
| `/calibrate` | Drift (paths exist, file counts match reality) |
| `/atlas-review` | **Quality/content** (this command) |
...
Run `/capture` to add missing patterns or anti-patterns.
Run `/explore --pattern {id}` to complete pattern guides.
```
...
| Missing files | List missing | Run /rechart |
...
**You review atlas quality, not structure. Use /validate for structure, /calibrate for drift.**
