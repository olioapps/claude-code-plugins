---
allowed-tools: Task, Bash(git status:*), Bash(git diff:*), Bash(git log:*), Bash(git branch:*), Bash(git rev-parse:*), Bash(git rev-list:*), Bash(git add:*)
argument-hint: [additional context]
description: Stage all changes and create a commit with AI-generated message
---

## Context

Create git commit by staging **all changes** and generating an AI-powered commit message using the `commit-writer` agent.

Arguments: `/git-actions:commit-all $ARGUMENTS`
...
**YOU execute staging using the Bash tool:**

```bash
git add -A
```

Then verify what was staged:
```bash
git status --short
```

**Error Handling:**
- If no changes exist after staging, inform user: "No changes to commit" and exit
- If staging fails, report the error and exit
...
1. ✅ Stage all files (`git add -A`) - requires user approval
...
**Staging fails:** report error details, exit workflow
